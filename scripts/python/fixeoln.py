...
Examples: 
        python fixeoln.py tounix /Stuff/websites/index.html     # on Unix, utf8
        python3 docetc/fixeoln.py todos somefile.py utf8        # on Unix
        py -3 docetc\fixeoln.py tounix ..\somefile.txt latin1   # on Windows
...
To apply this to many files: use a Unix find, or Python os.walk() or glob.glob.

Update: use the newer "fixeoln-all.py" to automatically apply this script to 
all text files in an entire folder tree.  That script applies a single Unicode 
type to every file in the tree; adjust types and handle failures as needed.
...
to single bytes or may falsely produce their ASCII values algorithmically (see
the coding note below: end-line bytes may be encoded per _any_ Unicode scheme).
...
encoded end-line character bytes in every possible Unicode encoding.
...
_very_ large files; to do better, write lines as they are read to a temp file.
...
def convertEndlines(mode, fname, encoding, reraise=False, debug=Debug):
    ...
    return changed   # tell caller what we found
